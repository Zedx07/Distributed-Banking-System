...
public class Server extends UnicastRemoteObject implements checkBal {
    static MongoClient client = MongoClients.create("mongodb+srv://Shubham:1234@cluster0.srmnd6r.mongodb.net/?retryWrites=true&w=majority");
...
//        UpdateResult doc = coll.updateOne(new Document("AccountNo", accNo),new Document("$set", new Document("Balance", bal)));
        UpdateResult doc = coll.updateOne(Filters.eq("AccountNo", accNo), Updates.set("Balance", bal));
...
//          System.out.println(doc.values());
            ...
            if(doc.get("Password").toString() == password){
                ...
                updateOnDatabase((Integer.parseInt(doc.get("AccountNo").toString())), debitnewBal);

                int credit_newBal = (Integer.parseInt(doc.get("Balance").toString())) + amt;
                updateOnDatabase(Integer.parseInt(cred_acc_no), credit_newBal);
                return true;
            }
            else {
